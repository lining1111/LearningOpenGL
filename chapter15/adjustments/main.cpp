...
//neighbor values that wrap
    double x2 = x1 - 1;
    if (x2 < 0) x2 = (round(noiseHeight / zoom)) - 1;
    double y2 = y1 - 1;
    if (y2 < 0) y2 = (round(noiseWidth / zoom)) - 1;
    double z2 = z1 - 1;
    if (z2 < 0) z2 = (round(noiseDepth / zoom)) - 1;
    ...
    for (int i = 0; i < noiseHeight; i++) {
        for (int j = 0; j < noiseWidth; j++) {
            for (int k = 0; k < noiseDepth; k++) {
                data[i * (noiseWidth * noiseHeight * 4) + j * (noiseHeight * 4) + k * 4 + 0] = (GLubyte) turbulence(i,
                                                                                                                    j,
                                                                                                                    k,
                                                                                                                    maxZoom);
                data[i * (noiseWidth * noiseHeight * 4) + j * (noiseHeight * 4) + k * 4 + 1] = (GLubyte) turbulence(i,
                                                                                                                    j,
                                                                                                                    k,
                                                                                                                    maxZoom);
                data[i * (noiseWidth * noiseHeight * 4) + j * (noiseHeight * 4) + k * 4 + 2] = (GLubyte) turbulence(i,
                                                                                                                    j,
                                                                                                                    k,
                                                                                                                    maxZoom);
...
GLuint buildNoiseTexture() {
    ...
    glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0, noiseWidth, noiseHeight, noiseDepth, GL_RGBA,
...
auto basePath = string("chapter15/adjustments/");

void init(GLFWwindow *window) {
    renderingProgramSURFACE = Utils::createShaderProgram(string(basePath + "vertShaderSURFACE.glsl").c_str(),
                                                         string(basePath + "fragShaderSURFACE.glsl").c_str());
    renderingProgramFLOOR = Utils::createShaderProgram(string(basePath + "vertShaderFLOOR.glsl").c_str(),
                                                       string(basePath + "fragShaderFLOOR.glsl").c_str());
    renderingProgramCubeMap = Utils::createShaderProgram(string(basePath + "vertCShader.glsl").c_str(),
                                                         string(basePath + "fragCShader.glsl").c_str());

    lightLoc = glm::vec3(0.0f, 5.0f, -30.0f);

    setupVertices();

    skyboxTexture = Utils::loadCubeMap(string(basePath + "cubeMap").c_str()); // expects a folder name
...
int main(void) {
    ...
    GLFWwindow *window = glfwCreateWindow(600, 600, "Program 15.6 - adding fog and Fresnel effects", NULL, NULL);
