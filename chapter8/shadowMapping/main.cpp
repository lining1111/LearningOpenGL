...
auto basePath = string("chapter8/shadowMapping/");
...
void setupShadowBuffers(GLFWwindow *window) {
    ...
    // may reduce shadow border artifacts 夹紧到边缘
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
}

//执行调用着色器并初始化物体
//调用setupShadowBuffers()函数初始化阴影贴图相关缓冲区
//最后，构造矩阵B进行从光照空间到纹理空间的转换
void init(GLFWwindow *window) {
    renderingProgram1 = Utils::createShaderProgram(string(basePath + "vert1Shader.glsl").c_str(),
                                                   string(basePath + "frag1Shader.glsl").c_str());
    renderingProgram2 = Utils::createShaderProgram(string(basePath + "vert2Shader.glsl").c_str(),
                                                   string(basePath + "frag2Shader.glsl").c_str());
...
void display(GLFWwindow *window, double currentTime) {
    ...
    glEnable(GL_POLYGON_OFFSET_FILL);    //减少阴影伪影
...
void passOne(void) {
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo[4]);//索引缓冲区
...
void passTwo(void) {
    ...
    //绘制环面，这次我们需要加入光照、材质、法向量等，同时我们需要为相机空间以及光照空间都提供MVP矩阵
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo[4]);//环面索引
    glDrawElements(GL_TRIANGLES, numTorusIndices, GL_UNSIGNED_INT, 0);
