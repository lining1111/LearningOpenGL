...
float rotAmt = 0.0f;
auto basePath = string("chapter17/sideBySide/");
ImportedModel ground(string(basePath + "grid.obj").c_str());
...
void init(GLFWwindow *window) {
    ...
    rockyTexture = Utils::loadTexture(string(basePath + "bkgd1.jpg").c_str());
    heightMap = Utils::loadTexture(string(basePath + "height.jpg").c_str());
}

void computePerspectiveMatrix(float leftRight) {
    ...
    pMat = glm::frustum(left, right, bottom, top, near, far);
}
...
int main(void) {
    ...
    GLFWwindow *window = glfwCreateWindow(1920, 1080, "Chapter 17 - program 2", NULL, NULL);
