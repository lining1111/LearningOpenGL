...
double smoothNoise(double zoom, double x1, double y1, double z1) {
    ...
    //neighbor values
    double x2 = x1 - 1;
    if (x2 < 0) x2 = round(noiseWidth / zoom) - 1.0;
    double y2 = y1 - 1;
    if (y2 < 0) y2 = round(noiseHeight / zoom) - 1.0;
    double z2 = z1 - 1;
    if (z2 < 0) z2 = round(noiseDepth / zoom) - 1.0;
...
auto basePath = string("chapter14/cloudsChanging/");
...
void display(GLFWwindow *window, double currentTime) {
    ...
    rotAmt += (float) ((currentTime - prevTime) * 0.1);
    mMat = glm::rotate(mMat, rotAmt, glm::vec3(0.0f, 1.0f, 0.0f));
    mvMat = vMat * mMat;

    depth += (float) ((currentTime - prevTime) * 0.003f);
    if (depth >= 0.999f) depth = 0.001f;
    cout << depth << endl;
...
int main(void) {
    ...
    GLFWwindow *window = glfwCreateWindow(800, 800, "Chapter 14 - program 8", NULL, NULL);
